...
twobits.directive('[data-properties]', element => {
    const config = map(
        element.getAttribute('data-properties').split(/\s+/),
        value => value.split(':')
    );

    return function(get) {
        forEach(config, ([elementProp, dataProp]) => element[elementProp] = get(dataProp));
    };
});
twobits.directive('[data-view]', (element, view) => {
    if (element === view.element) { return noop; }
    const { children, instantiates } = view;
    const config = element.getAttribute('data-view').split(':');
    let prop = config[0];
    let className = config[1];
 ...
 *  data-properties
 *  ---------------
 *  Much like the ```data-attributes``` directive, the ```data-properties``` directive can be used
 *  to create bindings between property values and HTML element property values.
 *
 *  ```html
 *  <input type="checkbox" data-properties="checked:state.isChecked" />
 *  ```
 *
 *  data-view
 *  ---------
 ...
 *
 *  It is possible to specify just the name of a class for this directive. In that case, an
 *  instance of that class will be created, but it will not be made the value of a property on the
 *  `TemplateView`.
 *
 *  ```javascript
...
export default class TemplateView extends View {
    constructor(element) {
        ...
        /**
         * If set to true, the element's own attributes will be updated with the data passed to its
         * `update()` method.
         *
         * @property updateSelf
         * @type Boolean
     ...
     **********************************************************************************************/
    /**
     * The data the view is currently rendering. If update() has not been called yet, it will be an
     * empty object (`{}`.)
     *
     * The returned object is recursively frozen and the property is not settable.
     *
     * @property data
     * @type Object
     * @default {}
     ...
     **********************************************************************************************/
    /**
     * Makes twobits.js update the DOM with the provided data. Also updates the data property.
     *
     * @method update
     *
     * @param {Object} data
    ...
    update(data) {
        ...
        _(this).data = freeze(extend(_(this).data, data));
    ...
    didCreateElement() {
        ...
        const options = {
            ...
            filter: node => {
                ...
                return !parent ||
                    parent === this.element ||
                    !(views.get(parent) instanceof TemplateView);
            }
        };
        const compilers = this.updateSelf ?
            [twobits.parse(this.element, options)] :
            map(children, child => twobits.parse(child, options));
